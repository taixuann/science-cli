...
def ls_handler(args: list) -> None:
    ...
    # -m study — list studies (no project context required)
    if mode == "study":
        _ls_study(pos, flags)
        return

    # -m instrument — list instruments (no project context required)
    if mode == "instrument":
        _ls_instrument(pos, flags)
        return

    # -m device — list device types
    if mode == "device":
        _ls_device(pos, flags)
        return

    # -m grammar — list grammar patterns (no project context required)
    if mode == "grammar":
        _ls_grammar(pos, flags)
        return
...
def _ls_default(proj: Path) -> None:
    ...
    for py in proto_yamls:
        ...
        devices = data.get("devices", "general")
        n_files = sum(len(s.get("files", [])) for s in steps)
        table.add_row(py.stem, devices, str(len(steps)), str(n_files), _fmt_desc(data.get("description", "")))
...
def _ls_protocol(proj: Path, show_all: bool = False, show_step: bool = False, show_files: bool = False) -> None:
    ...
    for py in proto_yamls:
        ...
        devices_val = data.get("devices", "general")
        # F3: Rich Table with bold title and cyan border
        table = Table(title=f"Protocol: {py.stem}  [dim](devices: {devices_val})[/dim]", border_style="cyan", show_lines=True)
        ...
        for s in steps:
            sn = s.get("name", "?")
            t = s.get("technique", "")
            d = s.get("instrument") or s.get("device", "")
            sfiles = s.get("files", [])
            n_files = len(sfiles)
        ...
        console.print(table)
        if devices_val and devices_val != "general":
            console.print(f"  Devices: [bold yellow]{devices_val}[/bold yellow]  [dim]{desc}[/dim]")
        else:
...
def _ls_study(pos: list, flags: dict) -> None:
    """List studies. Optional: specify study name for detail view."""
    technique_filter = flags.get("t") or flags.get("technique", "")
    name_filter = pos[0] if pos else None

    from science_cli.core.config import get_study_config, list_studies, load_global_config
    from science_cli.core.studies import parse_study_name

    if name_filter:
        cfg = get_study_config(name_filter)
        if not cfg:
            console.print(f"[yellow]Study '{name_filter}' not found.[/yellow]")
            return
        tech, sname = parse_study_name(name_filter)
        console.print(f"\n[bold]Study:[/bold] {name_filter}")
        console.print(f"  Technique:  {tech}")
        console.print(f"  Label:      {cfg.get('label', '')}")
        console.print(f"  Patterns:   {', '.join(cfg.get('patterns', []))}")
        instruments = cfg.get("instruments", {})
        if instruments:
            console.print(f"\n  [bold]Instruments:[/bold]")
            for ins_name, ins_cfg in instruments.items():
                delim = ins_cfg.get("delimiter", "auto")
                header = ins_cfg.get("header_lines", 0)
                console.print(f"    {ins_name:<20} delim={delim} header={header}")
    else:
        studies = list_studies(technique_filter or None)
        if not studies:
            console.print("[yellow]No studies configured.[/yellow]")
            return

        # Build reverse mapping: study -> list of device type labels
        config = load_global_config()
        device_types = config.get("device_types", {})
        study_to_dt_labels: dict[str, list[str]] = {}
        dt_labels: list[str] = []
        for dt_slug, dt_cfg in device_types.items():
            label = dt_cfg.get("label", dt_slug)
            dt_labels.append(label)
            for study_ref in dt_cfg.get("studies", []):
                if study_ref in studies:
                    study_to_dt_labels.setdefault(study_ref, []).append(label)

        # Group studies by device type label; unassigned go to "General"
        grouped: dict[str, list[str]] = {}
        for sname in sorted(studies):
            dt_matches = study_to_dt_labels.get(sname, ["General"])
            for dt_label in dt_matches:
                grouped.setdefault(dt_label, []).append(sname)

        # Sort groups: device types in order, "General" at the end
        def _group_sort_key(item: tuple[str, list[str]]) -> int:
            label, _ = item
            if label in dt_labels:
                return dt_labels.index(label)
            return len(dt_labels)

        sorted_groups = sorted(grouped.items(), key=_group_sort_key)

        for group_label, group_studies in sorted_groups:
            table = Table(title=group_label, border_style="cyan")
            table.add_column("Technique", style="green")
            table.add_column("Study", style="bold")
            table.add_column("Label")
            table.add_column("Instruments")
            for sname in group_studies:
                cfg = get_study_config(sname)
                tech, short_name = parse_study_name(sname)
                label = cfg.get("label", "")[:40] if cfg else ""
                instrs = ", ".join(cfg.get("instruments", {}).keys()) if cfg and cfg.get("instruments") else "\u2014"
                table.add_row(tech, short_name, label, instrs)
            console.print(table)


def _ls_instrument(pos: list, flags: dict) -> None:
    """List instruments. Optional: name for detail view."""
    ...
    if name_filter:
        ...
        if not ins:
            console.print(f"[yellow]Instrument '{name_filter}' not found.[/yellow]")
            return
        console.print(f"\n[bold]Instrument:[/bold] {name_filter}")
        console.print(f"  Label:       {ins.get('label', '')}")
        console.print(f"  Location:    {ins.get('location', '')}")
        console.print(f"  Type:        {ins.get('type', '')}")
        console.print(f"  Techniques:  {', '.join(ins.get('techniques', []))}")
        ...
        if not instruments:
            console.print("[yellow]No instruments registered.[/yellow]")
            return
        title = "Instruments" + (f" (filter: {study_filter})" if study_filter else "")
        ...
        for ins in instruments:
            if study_filter and study_filter not in ins.get("techniques", []):
                continue
            table.add_row(
                ins.get("name", ""),
                ins.get("label", ""),
                ins.get("type", ""),
                ins.get("location", ""),
                ", ".join(ins.get("techniques", [])),
            )
        console.print(table)


def _ls_device(pos: list, flags: dict) -> None:
    """List device types with their associated studies."""
    ...
    if not device_types:
        console.print("[yellow]No device types configured.[/yellow]")
        return

    table = Table(title="Device Types", border_style="cyan")
    table.add_column("Name", style="bold")
    table.add_column("Studies")

    for name in sorted(device_types.keys()):
        dt = device_types[name]
        studies_list = dt.get("studies", [])
        studies_str = "\n".join(studies_list) if studies_list else "\u2014"
        table.add_row(name, studies_str)

    console.print(table)


def _ls_grammar(pos: list, flags: dict) -> None:
    """List file naming grammar patterns."""
    from science_cli.core.config import get_file_naming_patterns, get_device_type_grammar

    device_type = flags.get("device-type") or flags.get("dt", "")
    if device_type:
