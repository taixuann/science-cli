...
def edit_handler(args: list) -> None:
    ...
    if sub == "-m":
        if not sub_args:
            console.print("[yellow]Usage: edit -m protocol|metadata|data|study|instrument|device|grammar [flags][/yellow]")
            return
        mode = sub_args[0]
        ...
        if mode == "protocol":
            ...
            _edit_study(mode_args)
        elif mode == "instrument":
            _edit_instrument(mode_args)
        elif mode == "device":
            _edit_device(mode_args)
        elif mode == "grammar":
            _edit_grammar_cmd(mode_args)
        else:
...
def _edit_protocol(args: list) -> None:
    ...
    study_raw = flags.get("s") or flags.get("study", "")
    devs_raw = flags.get("ins") or flags.get("instrument") or flags.get("d") or flags.get("device")
    if devs_raw and not flags.get("ins") and not flags.get("instrument"):
        console.print("[yellow]⚠️  '-d/--device' is deprecated. Use '--ins/--instrument' instead.[/yellow]")
    # If study flag is given and technique is not, derive technique from study
    if study_raw and not techs_raw:
        from science_cli.core.config import resolve_technique_from_study
        techs_raw = resolve_technique_from_study(study_raw)
        console.print(f"  [dim]Derived technique '{techs_raw}' from study '{study_raw}'[/dim]")
    param_raw = flags.get("param")
    devices_flag = flags.get("devices")
    ...
    if steps_raw:
        ...
        for i, sn in enumerate(step_names):
            if sn in existing_steps:
                if study_raw:
                    existing_steps[sn]["study"] = study_raw
                if i < len(techs):
                    existing_steps[sn]["technique"] = techs[i]
                if i < len(devs):
                    existing_steps[sn]["instrument"] = devs[i]
            else:
                entry = {"name": sn}
                if study_raw:
                    entry["study"] = study_raw
                if i < len(techs):
    ...
    if devices_flag:
        from science_cli.core.protocol import validate_device_type
        if not validate_device_type(devices_flag):
            from science_cli.core.protocol import KNOWN_DEVICE_TYPES
            console.print(f"[yellow]Warning: unknown device type '{devices_flag}'. Known: {', '.join(KNOWN_DEVICE_TYPES)}[/yellow]")
        data["devices"] = devices_flag
...
def _edit_study(args: list) -> None:
    """Edit a study entry — opens config-devices.yaml in $EDITOR."""
    if not args:
        console.print("[yellow]Usage: edit -m study <name>[/yellow]")
        return
    config_dir = Path.home() / ".config" / "science-cli"
    path = config_dir / "config-devices.yaml"
    if not path.exists():
        from science_cli.core.config_defaults import generate_config_devices_yaml
        config_dir.mkdir(parents=True, exist_ok=True)
        with open(path, "w") as f:
            f.write(generate_config_devices_yaml())
...
def _edit_instrument(args: list) -> None:
    """Edit an instrument entry — opens config-instruments.yaml in $EDITOR."""
    config_dir = Path.home() / ".config" / "science-cli"
...
def _edit_device(args: list) -> None:
    """Edit a device entry — opens config-devices.yaml in $EDITOR."""
    config_dir = Path.home() / ".config" / "science-cli"
    path = config_dir / "config-devices.yaml"
    if not path.exists():
        from science_cli.core.config_defaults import generate_config_devices_yaml
