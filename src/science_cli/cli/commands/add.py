...
def _add_protocol(args: list) -> None:
        ...
        for i, sn in enumerate(step_names):
            ...
            if i < len(devs) and devs[i] and (i >= len(techs) or not techs[i]):
                ...
                if compatible:
                    ...
                    console.print(f"  [dim]Auto-assigned technique '{compatible[0]}' from instrument '{devs[i]}'[/dim]")
            steps.append(entry)
    ...
    protocol = {
        "name": safe_name,
        "devices": devices_flag if devices_flag else "general",
        "description": desc,
...
def _add_metadata(args: list) -> None:
    ...
    if devs:
        parts.append(f"instruments: {', '.join(devs)}")
