...
HELP_SECTIONS = {
    ...
    "GROUP 2: CONTEXT & INFORMATION": ["open", "close", "init", "status", "results", "info", "chat"],
    "GROUP 3: LIBRARY PLOTTING": ["plot", "analyze", "---", "serve", "---", "iv", "pulse", "pvd", "raman", "uv-vis", "ec", "afm"],
...
COMMAND_DESCRIPTIONS = {
    ...
    "init":    "Initialize config files (standalone, was 'config init')",
    "plot":    "Generic plot — fzf-based, auto-detects technique (supports --study)",
    "analyze": "Generic analyze — fzf-based, technique-aware (supports --study)",
...
COMMAND_HELP: Dict[str, dict] = {
    "add": {
        ...
        "desc": "Add project/protocol/metadata/data (Group 1). Supports --study flag.",
        "subcommands": {
        ...
        "flags": {
            "OPERATION": {
                ...
                "-s, --study":    {"desc": "Study name; derives technique if -t omitted"},
                "--ins, --instrument":   {"desc": "Instrument name(s) per step, comma-separated (e.g. keithley-2400)"},
                "-pt, --protocol":{"desc": "Protocol name (for metadata mode)"},
                "--all":          {"desc": "Assign all selected files to one step (skip per-file prompt)"},
                "--devices":      {"desc": "Device type for protocol (e.g. volatile-memristor)"},
            },
        },
        "examples": [
            "add -m project my-project",
            "add -m protocol -n doping --step 1_deposition,2_characterization -s ec:ec-ca,ec:ec-cv --ins keithley-2400,keysight-b1500a",
            "add -m protocol -n doping --step 1_iv -s iv:iv-bipolar-sweep",
            "add -m metadata --step 1_deposition -pt doping -t ec-ca --ins keithley-2400",
    ...
    "edit": {
        ...
        "desc": "Edit protocol/metadata/data/study/instrument/device/grammar (Group 1). Supports --study flag.",
        "subcommands": {
            "edit -m protocol":    {"desc": "Edit protocol name/description/steps/techniques/instruments, remove or reorder steps", "usage": "edit -m protocol -n <name> [--nn <new-name>] [--desc <text>] [--step s1,s2] [-t ec-cv,ec-ca] [--ins keithley-2400] [--rm-step <name>] [--reorder s1,s2,...]"},
            "edit -m metadata":    {"desc": "Edit file assignments, techniques, and instruments", "usage": "edit -m metadata -n <name> [--step s1] [-t ec-cv] [--ins keithley-2400] [--files f1.txt,f2.txt]"},
            "edit -m data":        {"desc": "Move/reassign files between steps (interactive fzf)", "usage": "edit -m data"},
            "edit -m study":       {"desc": "Edit study config (opens config-devices.yaml)", "usage": "edit -m study <name>"},
            "edit -m instrument":  {"desc": "Edit instrument config (opens config-instruments.yaml)", "usage": "edit -m instrument <name>"},
            "edit -m device":      {"desc": "Edit device config (opens config-devices.yaml)", "usage": "edit -m device <name>"},
            "edit -m grammar":     {"desc": "Edit grammar patterns (opens config-grammar.yaml)", "usage": "edit -m grammar"},
        },
        "flags": {
            "OPERATION": {
                ...
                "-t, --technique":{"desc": "Technique(s), comma-separated"},
                "-s, --study":    {"desc": "Study name; derives technique if -t omitted"},
                "--ins, --instrument":   {"desc": "Instrument name(s) per step, comma-separated"},
        ...
        "examples": [
            ...
            "edit -m data",
            "edit -m study iv:iv-bipolar-sweep",
            "edit -m instrument keithley-2400",
            "edit -m device",
            "edit -m grammar",
        ],
    },
    ...
    "ls": {
        "usage": "ls [flags] [<step>]",
        "desc": "List projects/protocols/steps/files/studies/instruments/grammar (Group 1). Global, not session-bound.",
        "subcommands": {
            "ls":                   {"desc": "List all protocols and steps", "usage": "ls"},
            "ls -m project":     {"desc": "List all projects (was 'project list')", "usage": "ls -m project"},
            "ls -m study":       {"desc": "List studies (optionally -t <technique> filter)", "usage": "ls -m study [<name>]"},
            "ls -m instrument":  {"desc": "List instruments (optionally -s <study> filter)", "usage": "ls -m instrument [<name>]"},
            "ls -m device":      {"desc": "Alias for 'ls -m instrument'", "usage": "ls -m device [<name>]"},
            "ls -m grammar":     {"desc": "List grammar patterns", "usage": "ls -m grammar [--device-type <type>]"},
        ...
        "flags": {
            "OUTPUT": {
                "--json":        {"desc": "Output as JSON (machine-readable)"},
            },
        },
        "examples": [
            "ls",
            "ls -m project",
            "ls -m study",
            "ls -m study iv:iv-bipolar-sweep",
            "ls -m instrument",
            "ls -m instrument keithley-2400",
            "ls -m grammar",
            "ls -m protocol",
            "ls -m protocol --all",
    ...
    "raman": {
        ...
        "desc": "Raman spectroscopy: list, inspect, analyze (Group 3). Use 'sci plot --study raman:raman-spectrum' for plotting.",
        "subcommands": {
        ...
        "examples": [
            ...
            "plot --study raman:raman-spectrum 260526_PDA-ITO_raman_01.txt --name raman_spectrum.pdf       # preferred",
            "raman plot 260526_PDA-ITO_raman_01.txt --name raman_spectrum.pdf                   # deprecated",
            "raman analyze --ai",
    ...
    "uv-vis": {
        ...
        "desc": "UV-Vis spectroscopy: list, inspect, analyze (Group 3). Use 'sci plot --study uv-vis:uv-vis-spectrum' for plotting.",
        "subcommands": {
        ...
        "examples": [
            "uv-vis ls",
            "uv-vis info",
            "plot --study uv-vis:uv-vis-spectrum 260526_PDA-ITO_uvvis_01.txt --name uv_vis.pdf       # preferred",
            "uv-vis plot 260526_PDA-ITO_uvvis_01.txt --name uv_vis.pdf                   # deprecated",
            "uv-vis analyze",
        ],
    },
    "ec": {
        "usage": "ec <subcommand> [args...]",
        "desc": "Electrochemistry CV/CA/EIS: list, inspect, analyze (Group 3). Use 'sci plot --study' for plotting.",
        "subcommands": {
        ...
        "examples": [
            "ec ls",
            "ec info",
            "plot --study ec:ec-cv file.csv       # preferred (was 'ec plot')",
            "ec analyze --peaks --charge",
            "ec analyze --circuit RRC --kk",
        ],
    },
    "init": {
        "usage": "init [--global|--project|--monolithic]",
        "desc": "Initialize config files (Group 2). Standalone alternative to 'config init'. Creates modular config files under ~/.config/science-cli/.",
        "subcommands": {
            "init --global":     {"desc": "Write modular config to ~/.config/science-cli/ (default)", "usage": "init --global"},
            "init --project":    {"desc": "Write sci-config.yaml to current project", "usage": "init --project"},
            "init --monolithic": {"desc": "[DEPRECATED] Create old-style single config.yaml", "usage": "init --monolithic"},
        },
        "flags": {
            "--global":     {"desc": "Write to ~/.config/science-cli/ (4 modular files)"},
            "--project":    {"desc": "Write to <project_root>/sci-config.yaml"},
            "--monolithic": {"desc": "[DEPRECATED] Create old-style single config.yaml"},
        },
        "examples": [
            "init",
            "init --global",
    ...
    "plot": {
        "usage": "plot [--studies <study>] [--overlay/--all] [flags] [<file>...]",
        "desc": "Plot data — fzf-based, auto-detects technique (Group 3). Use -s/--study to set study context.",
        "subcommands": {
            ...
            "plot <file>":         {"desc": "Direct: plot file(s) with auto-detected technique", "usage": "plot file.csv"},
            "plot --studies <study>": {"desc": "Plot with explicit study context", "usage": "plot --studies iv:iv-bipolar-sweep file.csv"},
        ...
        "flags": {
            "STUDY / TECHNIQUE": {
                "-s, --study":    {"desc": "Study name (e.g. 'iv:iv-bipolar-sweep', 'pulse:pulse-stp-decay'). Derives technique automatically."},
                "-t, --technique":{"desc": "(deprecated) Technique type. Use -s/--study instead."},
                "--device-type, -dt": {"desc": "Device type override (e.g. volatile-memristor, non-volatile-memristor)"},
            },
            "THEME — COMMON": {
                ...
                "-n, --name": {"desc": "Output filename: .svg, .pdf, .png"},
            },
            "OUTPUT": {
        ...
        "examples": [
            ...
            "plot --study iv:iv-bipolar-sweep file.csv --gradient",
            "plot --study pulse:pulse-stp-decay file.csv --describe",
            "plot file.csv",
        ],
    },
    "analyze": {
        "usage": "analyze [options] [<file>...]",
        "desc": "Analyze data — unified, study-aware (Group 3). Use -s/--study to set study context and unlock flags. Use --technique for backward compat.",
        "subcommands": {
            "analyze":                    {"desc": "Interactive: fzf file selection, auto-detects technique", "usage": "analyze"},
            "analyze --study <study>":    {"desc": "Direct: analyze with explicit study context", "usage": "analyze --study raman:raman-spectrum --peaks"},
        },
        "flags": {
            "STUDY / TECHNIQUE": {
                "-s, --study":     {"desc": "Study name (e.g. 'iv:iv-bipolar-sweep', 'pulse:pulse-stp-decay'). Derives technique automatically."},
            ...
            "IV-SWEEP (with --study iv:iv-bipolar-sweep)": {
                "--yaml":     {"desc": "Output analysis YAML to results/"},
                "--vset-only":{"desc": "Volatile mode: V_set only"},
                "--compliance":{"desc": "Compliance current (A)"},
            },
            "RAMAN (with --study raman:raman-spectrum)": {
                "--yaml":     {"desc": "Output analysis YAML"},
                "--peaks":    {"desc": "Find and report peaks"},
                "--baseline": {"desc": "Baseline method: poly|asls|airpls"},
            },
            "UV-VIS (with --study uv-vis:uv-vis-spectrum)": {
                "--yaml":     {"desc": "Output analysis YAML"},
                "--bandgap":  {"desc": "Compute Tauc bandgap"},
            },
            "AFM (with --study afm:afm-topography)": {
                "--yaml":     {"desc": "Output analysis YAML"},
                "--roughness":{"desc": "Compute Sa/Sq roughness"},
            },
        },
        "examples": [
            "analyze",
            "analyze --study raman:raman-spectrum --peaks",
            "analyze -t iv-sweep --vset-only",
            "analyze -s iv:iv-bipolar-sweep --vset-only",
            "analyze -t uv-vis --bandgap",
            "analyze -t afm --roughness --yaml",
        ],
    },
    "instrument": {
        "usage": "instrument <subcommand> [args...]",
        "desc": "[DEPRECATED] Instrument model registry — use 'ls -m instrument' and 'edit -m instrument' instead (Group 3).",
        "subcommands": {
            "instrument ls <study>":  {"desc": "List models compatible with a study", "usage": "instrument ls pulse:pulse-stp-decay"},
            "instrument info <name>":      {"desc": "Show instrument model details (config, techniques)", "usage": "instrument info keithley-2400"},
            "instrument register <name>":  {"desc": "Register a new instrument model (opens $EDITOR)", "usage": "instrument register my-instrument"},
            "instrument edit <name>":      {"desc": "Edit instrument model config (opens $EDITOR)", "usage": "instrument edit keithley-2400"},
            "instrument rm <name>":        {"desc": "Remove instrument model from registry (requires --confirm)", "usage": "instrument rm keithley-2400 --confirm"},
        },
        "examples": [
            "instrument ls",
            "instrument ls iv:iv-bipolar-sweep",
            "instrument info keithley-2400",
            "instrument register custom-instrument",
            "instrument edit keithley-2400",
            "instrument rm old-instrument --confirm",
        ],
    },
    "afm": {
        "usage": "afm <subcommand> [args...]",
        "desc": "AFM/SPM image analysis: list, inspect, analyze, export (Group 3). Use 'sci plot --study afm:afm-topography' for plotting.",
        "subcommands": {
        ...
        "examples": [
            ...
            "plot --study afm:afm-topography sample.gwy --colormap terrain --name topography.pdf       # preferred",
            "afm plot sample.gwy --cmap terrain --name topography.pdf                        # deprecated",
            "afm analyze",
    ...
    "config": {
        ...
        "desc": "[DEPRECATED] Configure themes, techniques, devices, and naming grammar — use 'init', 'ls -m', 'edit -m' instead (Group 2).",
        "subcommands": {
            ...
            "config list grammar":                          {"desc": "List naming grammar patterns (optionally per device-type)", "usage": "config list grammar [--device-type <type>]"},
            "config list grammar --device-type":             {"desc": "List grammar for a specific device type", "usage": "config list grammar --device-type memristor"},
            ...
            "config edit grammar":                          {"desc": "Edit naming grammar in $EDITOR", "usage": "config edit grammar [--device-type <type>|--protocol <name>]"},
            "config edit grammar --device-type":             {"desc": "Edit grammar for a specific device type", "usage": "config edit grammar --device-type memristor"},
            "config edit grammar --protocol":                {"desc": "Edit grammar for a specific protocol", "usage": "config edit grammar --protocol 1_iv-test"},
            ...
            "config grammar list":                          {"desc": "List naming grammar patterns", "usage": "config grammar list [--device-type <type>]"},
            "config grammar edit":                          {"desc": "Edit naming grammar in $EDITOR", "usage": "config grammar edit [--device-type <type>|--protocol <name>]"},
        },
        "flags": {
            "GLOBAL": {
                "config init":                              {"desc": "Generate a default config file", "usage": "config init [--global|--project]"},
        ...
        "examples": [
            ...
            "config edit --global",
            "config edit devices",
            "config edit grammar --device-type memristor",
            "config edit grammar --protocol 1_iv-test",
            "config devices list",
        ],
    },
}
...
def show_top_help() -> None:
    ...
    for section_name, cmd_names in HELP_SECTIONS.items():
        ...
        for name in cmd_names:
            if name == "---":
                console.print(f"    [dim]── study-specific[/dim]")
                continue
...
def _render_plot_subcommands(subcmds: dict) -> None:
    """Render plot subcommands with per-study flags indented under each study line."""
    from science_cli.plot.registry import STUDY_FLAGS
    ...
    _STUDY_ORDER = [
        ("raman:raman-spectrum", "Raman spectroscopy", None),
        ("ec:ec-cv", "Cyclic voltammetry", None),
        ("ec:ec-ca", "Chronoamperometry", None),
        ("ec:ec-eis", "Electrochemical impedance", None),
        ("uv-vis:uv-vis-spectrum", "UV-Vis spectroscopy", None),
        ("afm:afm-topography", "AFM/SPM topography", None),
        ("iv:iv-bipolar-sweep", "DC I-V sweep (bipolar/volatile)", [
            ("--gradient <cmap>", "Color sweep points by cycle position (default: viridis)"),
            ("--cmap <name>", "Colormap: viridis, plasma, rainbow"),
        ]),
        ("pulse:pulse-stp-decay", "Short-term plasticity (STP) decay", [
            ("--describe", "Show pulse parameters (width, max V/I, steady current)"),
        ]),
        ("pulse:pulse-ppf", "Paired-pulse facilitation (PPF)", [
            ("--describe", "Show pulse parameters (width, max V/I, steady current)"),
        ]),
        ("pulse:pulse-endurance", "Pulse endurance cycling", None),
        ("pulse:pulse-retention", "Pulse retention over time", None),
    ]

    # ── GROUP 1: COMMON ──
    console.print("  [bold]GROUP 1 — COMMON[/bold]")
    common_keys = ["plot", "plot --overlay", "plot --all", "plot <file>",
                   "plot results", "plot open <name>", "plot delete <name>"]
    for name, sub in subcmds.items():
        if name in common_keys:
            console.print(f"    [bold]{name:<45}[/bold] [dim]{sub['desc']}[/dim]")

    # ── GROUP 2: STUDIES ──
    console.print()
    console.print("  [bold]GROUP 2 — STUDIES[/bold]")
    console.print("    [dim]plot --studies <study-name> [--overlay/--all][/dim]")
    for name, sub in subcmds.items():
        if name == "plot --studies <study>":
            for study, desc, flags in _STUDY_ORDER:
                console.print(f"    [bold]--study {study:<38}[/bold] [dim]{desc}[/dim]")
                if flags:
                    for f_name, f_desc in flags:
                        console.print(f"      {f_name:<42} [dim]{f_desc}[/dim]")
...
def show_command_help(cmd: str) -> None:
    ...
    if subcmds:
        ...
        if cmd == "plot":
            _render_plot_subcommands(subcmds)
        else:
            for name, sub in subcmds.items():
            ...
            for category, cat_flags in flags.items():
                ...
                if len(cat_flags) > 10:
                    ...
                    for name, flag in cat_flags.items():
                        t = Text()
                        t.append(f"{name:<25}", style="bold")
                        t.append(f" {flag['desc']}", style="dim")
                        items.append(t)
                    cols = Columns(items, equal=True, column_first=True)
                    console.print(cols)
                else:
                    for name, flag in cat_flags.items():
