...
import numpy as np
from science_cli.library.pulse.models import RetentionData


def analyze_retention(time, resistance):
    """Analyze retention time series with log-time and power-law models."""
    ...
    if len(t) < 3:
        return {"decay_rate": None, "decay_model": None, "extrapolated_10yr": None,
                "lifetime_hours": None, "r_squared": None, "error": "Insufficient data points (need >=3)"}
    ...
    return {
        "decay_rate": decay_rate, "decay_model": decay_model,
        "extrapolated_10yr": extrapolated_10yr,
        "lifetime_hours": lifetime_hours, "r_squared": r_squared,
        "test_duration_hours": float(t[-1] / 3600.0), "n_points": int(len(t)),
    }
...
def retention_summary(data):
    ...
    lines = [f"Retention @ {data.temperature} K: {stats['test_duration_hours']:.1f} h test, {stats['n_points']} points"]
    lines.append(f"  Model: {stats['decay_model']} (R-squared={stats['r_squared']:.4f})")
    lines.append(f"  Decay rate: {stats['decay_rate']:.3e} Ohm/decade")
    if stats["extrapolated_10yr"] is not None:
        lines.append(f"  R(10yr): {stats['extrapolated_10yr']:.1f} Ohm")
    if stats["lifetime_hours"] is not None:
        lines.append(f"  Lifetime: {stats['lifetime_hours']:.0f} h ({stats['lifetime_hours']/24:.0f} days)")
    else:
        lines.append("  Lifetime: unable to estimate")
    return "\n".join(lines)


def analyze_retention_to_yaml(
    time,
    resistance,
    step_dir: Path,
    instrument: str = "",
    ...
    """Analyze retention and write YAML analysis file."""
    ...
    output = {
        "analysis": {
            ...
            "parameters": {
                "decay_rate": stats.get("decay_rate"),
                "decay_model": stats.get("decay_model"),
                "extrapolated_10yr": stats.get("extrapolated_10yr"),
                "lifetime_hours": stats.get("lifetime_hours"),
                "r_squared": stats.get("r_squared"),
                "test_duration_hours": stats.get("test_duration_hours"),
