...
.. deprecated::
    This module is a legacy shim. New code should use protocol YAML
    (``<protocol>.yaml``) instead of ``devices.yaml``. Study-aware
    technique resolution is handled by the CLI bridge handlers in
    ``cli/commands/`` — use ``--study``/``-s`` to pass study names.
"""
