...
def generate_iv_svg(
    ...
    with plt.rc_context(acs_rc):
        ...
        if flags:
            from science_cli.cli.commands.plot import _apply_figure_kw
...
def generate_iv_highlighted_svg(
    ...
    with plt.rc_context(nature_rc):
        ...
        if not use_log:
            ...
            # Compute tight limits directly from foreground (highlighted) traces to filter out background outliers
            fg_traces = [t for t in traces if t[2].get("order") in highlight_cycles]
            if not fg_traces:
                fg_traces = traces
            all_y = np.concatenate([t[1] for t in fg_traces if t[1] is not None and len(t[1]) > 0])
            all_x = np.concatenate([t[0] for t in fg_traces if t[0] is not None and len(t[0]) > 0])
