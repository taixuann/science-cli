...
def extract_resistance(
    ...
    se_slope = (
        np.sqrt(mse / np.sum((v_win - x_mean) ** 2))
        if mse > 0 and n > 2
        else 0.0
    )
...
def extract_scan_rate(
    ...
    if time is not None and len(time) > 1:
        ...
        dv_ = np.diff(voltage)
        sign_changes = np.sum(np.diff(np.sign(dv_)) != 0)
        n_sweeps = max(sign_changes + 1, 1)
        scan_rate = v_range / (n_sweeps + 1)
...
# ---------------------------------------------------------------------------
# Internal fit functions
# ---------------------------------------------------------------------------


def _fit_ohmic(voltage: np.ndarray, current: np.ndarray, result: dict) -> dict:
    ...
    result["metrics"]["aic"] = (
        float(n_fit * np.log(ss_res / n_fit) + 4)
        if n_fit > 0 and ss_res > 0
        else None
    )
    result["metrics"]["bic"] = (
        float(n_fit * np.log(ss_res / n_fit) + 4 * np.log(n_fit))
        if n_fit > 0 and ss_res > 0
        else None
    )
    result["success"] = True
...
def _fit_schottky(voltage: np.ndarray, current: np.ndarray, result: dict) -> dict:
...
def _fit_sclc(voltage: np.ndarray, current: np.ndarray, result: dict) -> dict:
...
def _fit_pool_frenkel(voltage: np.ndarray, current: np.ndarray, result: dict) -> dict:
...
def detect_sweep_segments(
    ...
    for i in range(len(boundaries) - 1):
        ...
        # Try to split at inner zero-crossing if segment is long enough
        if n_points >= 2 * min_segment_points:
            zv = v[s : e + 1]
            sv = np.sign(
                np.where(np.abs(zv) < 1e-12, 1e-12, zv)
            )
