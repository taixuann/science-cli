...
def analyze_eis(data: EISData, options: dict | None = None) -> dict:
    """Run selected EIS analyses. Options: circuit (str), kk (bool)."""
    if options is None:
        options = {}
    results: dict = {}
...
def _register_circuit(name: str, func: callable) -> None:
    """Register a circuit model function by name."""
    _circuit_functions[name] = func


# ---------------------------------------------------------------------------
# Circuit model functions — return Z(f) as complex array
# ---------------------------------------------------------------------------

def _rc_series(f: np.ndarray, R: float, C: float) -> np.ndarray:
    """Series RC circuit: Z = R + 1/(jωC)."""
    w = 2 * np.pi * f
    Z = R + 1.0 / (1j * w * C)
    return Z


def _rrc_randles(f: np.ndarray, Rs: float, Rct: float, Cdl: float) -> np.ndarray:
    """Randles circuit (R(RC)): Z = Rs + 1/(jωCdl + 1/Rct)."""
    w = 2 * np.pi * f
    Z = Rs + 1.0 / (1j * w * Cdl + 1.0 / Rct)
    return Z


def _rq_randles(f: np.ndarray, Rs: float, Rct: float, Q_mag: float, Q_n: float) -> np.ndarray:
    """Randles with CPE (R(RQ)): Z = Rs + 1/(1/Zcpe + 1/Rct)."""
    w = 2 * np.pi * f
...
# ---------------------------------------------------------------------------
# Circuit fitting via lmfit
# ---------------------------------------------------------------------------

def circuit_fit(data: EISData, circuit: str = "RRC") -> dict:
    """Fit an equivalent circuit model to EIS data using lmfit.

    Returns dict with keys: circuit, parameter_names, fitted_params,
    param_stderr, r_squared, reduced_chi, nfev, fit_Z_real, fit_Z_imag,
    fit_frequency.  On error, returns {'error': str}.
    """
    f = data.frequency
    Z = data.impedance
    ...
    # Set up lmfit parameters per circuit type
    ...
    try:
        ...
        stderr = [float(result.params[p].stderr if result.params[p].stderr is not None else 0) for p in param_names]

        Z_fit = func(f, **{p: result.params[p].value for p in param_names})

        residuals_abs = np.abs(Z_fit - Z)
        r_squared = 1 - (np.sum(residuals_abs ** 2) /
                         np.sum(np.abs(Z - np.mean(Z)) ** 2))
...
def kramers_kronig(data: EISData) -> dict:
    """Kramers-Kronig consistency check using a Voigt circuit model.

    Fits N RC elements (poles) in series to the data and checks how well
    the model (which is inherently KK-compliant) reproduces the data.

    Returns: {'passes': bool, 'consistency_score': float, 'n_poles': int,
              'reduced_chi': float}
    """
    f = data.frequency
    Z = data.impedance

    # Choose number of poles: roughly 7-10 per decade, but not too many
