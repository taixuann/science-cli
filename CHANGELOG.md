...
### Changed
- **`--describe` flag redesigned** — now embeds metadata INTO the plot figure instead of showing a separate waveform panel
  - `plot.py`: builds `describe_title` (V_HRS/V_LRS for endurance, study name fallback) and `describe_annotation` (compacted 2D waveform array `t=[...]µs / V=[...]V` + scalar metadata at figure bottom)
  - `generic.py`: renders `describe_title` as `fig.suptitle()` and `describe_annotation` as `fig.text()` in a rounded box at the bottom of the figure
  - `show_waveform` panel mode preserved for backward compatibility
- Terminal rich table output from `--describe` still works as before
