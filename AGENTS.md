...
| **keysight-b1500a/iv-sweep** | `~/.config/opencode/skills/keysight-b1500a/iv-sweep/SKILL.md` | Extract IV bipolar-sweep metadata from Keysight B1500A I/V List Sweep CSV files (V+, V-, compliance, step voltage, delay time). Reads header + data, cross-checks, writes to protocol.yaml. Bundled scripts: `scripts/extract_iv_metadata.py`, `scripts/update_protocol_metadata.py`. |
| **keysight-b1500a/endurance** | `~/.config/opencode/skills/keysight-b1500a/endurance/SKILL.md` | **(migrated from sci-keysight-endurance)** Parse Keysight WGFMU/B1500A endurance CSV files (v1+v2 formats), extract per-cycle LRS/HRS from Measurement Result sections, detect V_set/V_read voltages, compute R_LRS/R_HRS/ratio. Bundled script: `scripts/fix_extraction.py`. |
| **keithley-2400/iv-sweep** | `~/.config/opencode/skills/keithley-2400/iv-sweep/SKILL.md` | Extract IV bipolar-sweep metadata from Keithley 2400 LabVIEW LVM CSV files (V+, V-, compliance from data with plateau detection, step voltage, delay time from timestamps). Bundled scripts: `scripts/extract_iv_metadata.py`, `scripts/update_protocol_metadata.py`. |
| **sci-wgfmu-cycle-split** | `~/.config/opencode/skills/sci-wgfmu-cycle-split/SKILL.md` | Split multi-cycle WGFMU CSV files (8+ concatenated sub-files) into individual per-cycle files. Detects cycle boundaries via `TestRecord.RecordTime` markers. Creates valid self-contained WGFMU CSVs per cycle with `{remarks}` and `_NNN` count. Updates protocol.yaml and step-folder symlinks. Bundled script at `<skill>/scripts/split-wgfmu-cycles.py`. |
| **sci-waveform-pattern-store** | `~/.config/opencode/skills/sci-waveform-pattern-store/SKILL.md` | Store compressed waveform 2D patterns in protocol.yaml for STP pulse-decay files. Compresses 200-point dense patterns into key transition points (|ΔV| > 0.1V detection). Bundled script for batch processing step folders. |
| **sci-plan-workflow** | `~/.config/opencode/skills/sci-plan-workflow/SKILL.md` | **Auto-loaded by orchestrator agents.** On-ramp for all science-cli orchestration: task→skill routing table, session start protocol, delegation pipeline, domain skill reference. Load this FIRST before any feature planning in the science-cli workspace. |
